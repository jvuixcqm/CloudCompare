...
#ifdef Q_OS_WIN
#  ifndef NOMINMAX
#    define NOMINMAX   // 避免 windows.h 的 min/max 宏覆盖 std::min/max
#  endif
#  include <windows.h>   // for HeapCompact, GetProcessHeap, SetProcessWorkingSetSize
#endif

namespace {
// 强制 CRT / 进程堆将已 free 但仍保留在用户态的内存块归还操作系统。
// Windows 默认的低碎片堆 (LFH) 不会主动归还，连续切换大点云时会导致
// 工作集（Task Manager 看到的内存）持续上涨。这里在每次释放旧点云后
// 调用一次，让数百 MB 的点缓冲区真正退出工作集。
inline void trimProcessMemory()
{
#ifdef Q_OS_WIN
    HeapCompact(GetProcessHeap(), 0);
    // 把工作集压到当前最小驻留量，下一次访问页表会重新映射，平时不占物理内存
...
//! SRF / SUR / PLY / PCD：坐标+分辨率完全由文件自带，不依赖面板的分辨率/亮度图控件
inline bool isGridFormatPath(const QString& path)
{
    return TiffBmpLoader::isSrfPath(path)
        || TiffBmpLoader::isSurPath(path)
        || TiffBmpLoader::isPlyPath(path)
        || TiffBmpLoader::isPcdPath(path);
}
} // namespace
#ifndef M_PI
    ...
    , m_qcMode(qcMode)
{
...

    {
        ...
        m_tiffEdit->setPlaceholderText("选择或拖拽 TIFF / SRF / SUR / PLY / PCD 文件，或输入文件夹路径...");
        ...
        fileLay->addRow("TIFF/SRF/SUR/PLY/PCD:", row);
    }
    {
        ...
        m_bmpBrowseBtn = new QPushButton("...");
        m_bmpBrowseBtn->setFixedWidth(28);
        row->addWidget(m_bmpBrowseBtn);
    ...
    }
    {
        ...
        m_bmpFolderBtn = new QPushButton("...");
        m_bmpFolderBtn->setFixedWidth(28);
        row->addWidget(m_bmpFolderBtn);
    ...
    m_autoResChk = new QCheckBox("自动从文件名");
    m_autoResChk->setToolTip(
        "自动从文件名读取分辨率\n"
        "从文件名末尾 3 段下划线分隔的数值解析分辨率\n"
        "文件名格式: xxx_..._<X>_<Y>_<Z>.tif\n"
        "例: OUT5_16位TIFF_0.003_0.015_0.000100.tif\n"
        "（与「自动从TIFF标签」互斥，最多选一个）");

    m_autoTiffTagChk = new QCheckBox("自动从TIFF标签");
    m_autoTiffTagChk->setToolTip(
        "自动从 TIFF 标签读取分辨率\n"
        "读取 TIFF 内嵌 3D 标定元数据（ImageDescription / ModelPixelScale / XResolution）\n"
        "注意：部分 TIFF 的标签可能包含错误信息，可关闭此项\n"
        "（与「自动从文件名」互斥，最多选一个）");
...

    {
        ...
        chkRow->addWidget(m_autoTiffTagChk);
    ...
    // 上下箭头按钮的步进被限制在 [1, 100]（超出需手动输入）；setRange 仍是手动输入的完整范围
    m_zScaleSpin = new ButtonLimitedDoubleSpinBox(1.0, 100.0);
    m_zScaleSpin->setRange(0.01, 1000.0);
    // FlexibleDoubleSpinBox 默认用自适应步进（随数值增大自动放大步长），
    // 这里需要固定 +1/-1 的点击增量，改回标准步进类型
    m_zScaleSpin->setStepType(QAbstractSpinBox::DefaultStepType);
    m_zScaleSpin->setSingleStep(1.0);
    m_zScaleSpin->setValue(1.0);
    m_zScaleSpin->setSuffix(" x");
    m_zScaleSpin->setToolTip(
        "仅放大渲染显示的 Z 起伏，不改变真实坐标/单位\n"
        "测距、点坐标读数等工具仍使用真实 mm 值\n"
        "1 = 不放大（原始比例）\n"
        "上下箭头按钮的调节范围为 1~100，超出该范围请手动输入\n"
        "网格模式下放大时会临时关闭光照（改用纯颜色平画显示），\n"
        "避免非均匀缩放导致法线光照失真；恢复 1x 后自动恢复光照");
    m_zScaleResetBtn = new QPushButton("恢复1x");
    m_zScaleResetBtn->setToolTip("一键恢复 Z 显示放大为 1×（不放大）");
    {
        auto* zScaleRow = new QHBoxLayout;
        zScaleRow->setContentsMargins(0, 0, 0, 0);
        zScaleRow->setSpacing(4);
        m_lblZScaleText = new QLabel("Z显示放大:");
        zScaleRow->addWidget(m_lblZScaleText);
        zScaleRow->addWidget(m_zScaleSpin);
        zScaleRow->addWidget(m_zScaleResetBtn);
        zScaleRow->addStretch();
        modeLay->addLayout(zScaleRow);
    }
    ...
    m_statusLabel = new QLabel("就绪 — 支持拖拽 TIFF / SRF / SUR / PLY / PCD 文件");
    ...
    // ── 导出 TIFF 功能：折叠按钮在上、菜单在下，默认收起 ────────────────────
    // 按钮先 addWidget；菜单后 addWidget。这样视觉上按钮在上、菜单在下方展开
    m_exportEnableBtn = new QPushButton(QStringLiteral("▼  TIFF 转换导出"));
    m_exportEnableBtn->setCheckable(true);
    m_exportEnableBtn->setFixedHeight(26);
    m_exportEnableBtn->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
    m_exportEnableBtn->setToolTip("点击展开 / 收起「TIFF 转换导出」面板");
    mainLay->addWidget(m_exportEnableBtn);

    {
        m_exportGrp = new QGroupBox("TIFF 转换导出");
        auto* expLay = new QVBoxLayout(m_exportGrp);
        expLay->setSpacing(4);
        expLay->setContentsMargins(8, 8, 8, 8);

        // 输出目录行
        {
            auto* row = new QHBoxLayout;
            row->setContentsMargins(0, 0, 0, 0);
            m_lblExportDirRow = new QLabel("输出目录:");
            row->addWidget(m_lblExportDirRow);
            m_exportDirEdit = new QLineEdit;
            m_exportDirEdit->setPlaceholderText("默认 = 当前文件目录 / 转换TIFF输出目录");
            row->addWidget(m_exportDirEdit, 1);
            m_exportDirBtn = new QPushButton("...");
            m_exportDirBtn->setFixedWidth(28);
            row->addWidget(m_exportDirBtn);
            m_exportOpenBtn = new QPushButton(QStringLiteral("📂"));
            m_exportOpenBtn->setFixedWidth(28);
            m_exportOpenBtn->setToolTip(
                "在资源管理器中打开输出目录\n（不存在时自动创建「转换TIFF输出目录」）");
            row->addWidget(m_exportOpenBtn);
            expLay->addLayout(row);
        }
        // 格式 + 无效填充值
        {
            auto* row = new QHBoxLayout;
            row->setContentsMargins(0, 0, 0, 0);
            m_lblExportFmtRow = new QLabel("格式:");
            row->addWidget(m_lblExportFmtRow);
            m_exportFmtCb = new NoWheelComboBox;  // 滚轮事件被忽略，防止误选
            m_exportFmtCb->addItem("16-bit 无符号", 0);
            m_exportFmtCb->addItem("16-bit 有符号", 1);
            m_exportFmtCb->addItem("32-bit 浮点",   2);
            row->addWidget(m_exportFmtCb, 1);
            m_exportInvalidLbl = new QLabel("无效填充:");
            row->addWidget(m_exportInvalidLbl);
            m_exportInvalidSpin = new FlexibleDoubleSpinBox; // 同样不响应滚轮
            m_exportInvalidSpin->setRange(-1e9, 1e9);
            m_exportInvalidSpin->setValue(-100.0);
            m_exportInvalidSpin->setFixedWidth(80);
            m_exportInvalidSpin->setToolTip("32-bit 输出时 NaN 的填充值（默认 -100）");
            row->addWidget(m_exportInvalidSpin);
            expLay->addLayout(row);
        }
        // 导出按钮
        {
            auto* row = new QHBoxLayout;
            row->setContentsMargins(0, 0, 0, 0);
            m_exportOneBtn = new QPushButton("导出当前");
            m_exportOneBtn->setToolTip(
                "把当前加载的 TIFF / SRF / SUR / PLY / PCD 转换为目标格式 TIFF"
                "（非规则网格点云无法导出，按钮会自动禁用）");
            m_exportAllBtn = new QPushButton("自动遍历导出");
            m_exportAllBtn->setToolTip(
                "按当前排序方式自动遍历当前文件夹的所有 TIFF / SRF / SUR / PLY / PCD，逐个导出"
                "（非规则网格点云会自动跳过并计入失败）");
            row->addWidget(m_exportOneBtn, 1);
            row->addWidget(m_exportAllBtn, 1);
            expLay->addLayout(row);
        }
        mainLay->addWidget(m_exportGrp);
        m_exportGrp->setVisible(false);  // 默认隐藏，按钮点击展开
        // 初始：16-bit 无符号 → 隐藏无效填充控件
        m_exportInvalidLbl->setVisible(false);
        m_exportInvalidSpin->setVisible(false);
    }

    // 信号连接（按钮和菜单控件都已创建）
    {
        auto updateBtnText = [this](bool on) {
            // 注意：retranslateUi 也会刷新文字，此处的箭头随状态变化
            const QString base = ls("TIFF 转换导出", "TIFF Export");
            m_exportEnableBtn->setText((on ? QStringLiteral("▲  ") : QStringLiteral("▼  ")) + base);
        };
        connect(m_exportEnableBtn, &QPushButton::toggled, this,
                [this, updateBtnText](bool on) {
            if (m_exportGrp) m_exportGrp->setVisible(on);
            updateBtnText(on);
            scheduleSettingsSave();
            // 展开时延迟一拍把滚动条拉到底：先等布局重排（GroupBox 才有正确尺寸），
            // 再设到 max。延迟到事件循环空闲使用 invokeMethod + QueuedConnection
            if (on && m_scrollArea) {
        ...
        connect(m_exportFmtCb, QOverload<int>::of(&QComboBox::currentIndexChanged),
                this, [this](int idx) {
            const bool isFloat = (idx == 2);
            if (m_exportInvalidLbl)  m_exportInvalidLbl->setVisible(isFloat);
            if (m_exportInvalidSpin) m_exportInvalidSpin->setVisible(isFloat);
            scheduleSettingsSave();
        });
        connect(m_exportInvalidSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
                this, [this](double) { scheduleSettingsSave(); });
        connect(m_exportDirEdit, &QLineEdit::textChanged, this,
                [this](const QString&) { scheduleSettingsSave(); });
        connect(m_exportDirBtn, &QPushButton::clicked, this, [this] {
            const QString start = m_exportDirEdit->text().trimmed().isEmpty()
                ? QFileInfo(m_tiffEdit->text()).absolutePath()
                : m_exportDirEdit->text();
            const QString d = QFileDialog::getExistingDirectory(
                this, ls("选择 TIFF 输出根目录", "Select TIFF Output Root Folder"), start);
            if (!d.isEmpty()) m_exportDirEdit->setText(d);
        });
        connect(m_exportOpenBtn, &QPushButton::clicked, this, [this] {
            // 与导出时一致：自动确保 "转换TIFF输出目录" 子目录存在并打开
            const QString dir = ensureExportDir();
            if (dir.isEmpty()) {
                m_statusLabel->setText(ls("❌ 输出目录无效，无法打开",
        ...
        connect(m_exportOneBtn, &QPushButton::clicked,
                this, &TiffBmpPanel::onExportCurrent);
        connect(m_exportAllBtn, &QPushButton::clicked,
                this, &TiffBmpPanel::onExportAll);
    }
    ...
    connect(m_bmpBrowseBtn, &QPushButton::clicked, this, &TiffBmpPanel::onBrowseBmp);
    connect(m_bmpFolderBtn, &QPushButton::clicked, this, &TiffBmpPanel::onBrowseBmpFolder);
    ...
    connect(m_zScaleSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this, [this](double) {
                // 纯显示效果：直接对当前对象重新应用 GL 变换，不触发数据重新加载
                if (m_lastUID != 0 && m_app) {
                    ccHObject* obj = m_app->dbRootObject()->find(m_lastUID);
                    if (obj) {
                        applyZDisplayScale(obj);
                        m_app->redrawAll();
                    }
                }
                scheduleSettingsSave();
            });
    connect(m_zScaleResetBtn, &QPushButton::clicked,
            this, [this]() { m_zScaleSpin->setValue(1.0); });
    if (m_keepObjectChk)
    ...
    auto reapplyAutoRes = [this]() {
        const QString t = m_tiffEdit->text().trimmed();
        if (t.isEmpty() || !QFileInfo::exists(t)) return;
        bool applied = false;
        if (m_autoTiffTagChk->isChecked())
            applied = tryApplyEmbeddedMetadata(t);
        if (m_autoResChk->isChecked() && !applied)
            tryParseResolutionFromFilename(t);
        triggerAutoReload();
    };
    connect(m_autoResChk, &QCheckBox::toggled, this, [this, reapplyAutoRes](bool checked) {
        if (checked && m_autoTiffTagChk->isChecked()) {
            QSignalBlocker b(m_autoTiffTagChk);
            m_autoTiffTagChk->setChecked(false);
        }
        if (!checked && !m_autoTiffTagChk->isChecked())
            m_resWarningLabel->setVisible(false);
        if (checked) reapplyAutoRes();
        scheduleSettingsSave();
    });
    connect(m_autoTiffTagChk, &QCheckBox::toggled, this, [this, reapplyAutoRes](bool checked) {
        if (checked && m_autoResChk->isChecked()) {
            QSignalBlocker b(m_autoResChk);
            m_autoResChk->setChecked(false);
        }
        if (!checked && !m_autoResChk->isChecked())
            m_resWarningLabel->setVisible(false);
        if (checked) reapplyAutoRes();
    ...
    // F1（仅焦点在本面板/子控件上时生效，不占用全局快捷键）：
    // 隐藏开关——在"使用偏移"已勾选的前提下，切换16位无符号旧版兼容模式
    // （对 uint16 原始值做 -32768 重新解释，供修复前生成的旧TIFF使用）。
    // 状态只在本次运行内有效，不写入配置，重启后默认关闭。
    auto* scF1 = new QShortcut(Qt::Key_F1, this, nullptr, nullptr,
                               Qt::WidgetWithChildrenShortcut);
    connect(scF1, &QShortcut::activated, this, [this]() {
        if (!m_useOffsetChk || !m_useOffsetChk->isChecked()) {
            m_statusLabel->setText(ls(
                "⚠ 需先勾选\"使用偏移\"才能切换16位无符号旧版兼容模式",
                "⚠ Enable \"Use Offset\" first to toggle legacy uint16 compat mode"));
            m_statusLabel->setStyleSheet("color:#cc6600;");
            return;
        }
        m_legacyUint16CompatEnabled = !m_legacyUint16CompatEnabled;
        m_statusLabel->setText(m_legacyUint16CompatEnabled
            ? ls("✓ 已启用16位无符号旧版兼容模式（-32768，仅本次运行有效）",
                 "✓ Legacy uint16 compat mode ON (-32768, this session only)")
            : ls("已关闭16位无符号旧版兼容模式",
                 "Legacy uint16 compat mode OFF"));
        m_statusLabel->setStyleSheet(m_legacyUint16CompatEnabled
            ? "color:#cc6600;" : "color:#555;");
        triggerAutoReload();
    });
...
void TiffBmpPanel::dragEnterEvent(QDragEnterEvent* e)
{
    if (e->mimeData()->hasUrls()) {
        for (const QUrl& u : e->mimeData()->urls()) {
            ...
            if (p.endsWith(".tif") || p.endsWith(".tiff") || p.endsWith(".srf")
                || p.endsWith(".sur") || p.endsWith(".ply") || p.endsWith(".pcd")) {
...
void TiffBmpPanel::dropEvent(QDropEvent* e)
{
    ...
    for (const QUrl& u : e->mimeData()->urls()) {
        ...
        if (lp.endsWith(".tif") || lp.endsWith(".tiff") || lp.endsWith(".srf")
            || lp.endsWith(".sur") || lp.endsWith(".ply") || lp.endsWith(".pcd")) {
...
void TiffBmpPanel::onBrowseTiff()
{
    const QString path = QFileDialog::getOpenFileName(
        this,
        ls("选择 TIFF / SRF / SUR / PLY / PCD 文件", "Select TIFF / SRF / SUR / PLY / PCD file"),
        QFileInfo(m_tiffEdit->text()).absolutePath(),
        ls("TIFF / SRF / SUR / PLY / PCD (*.tif *.tiff *.srf *.sur *.ply *.pcd);;TIFF (*.tif *.tiff);;SRF (*.srf);;SUR (*.sur);;PLY (*.ply);;PCD (*.pcd)",
           "TIFF / SRF / SUR / PLY / PCD (*.tif *.tiff *.srf *.sur *.ply *.pcd);;TIFF (*.tif *.tiff);;SRF (*.srf);;SUR (*.sur);;PLY (*.ply);;PCD (*.pcd)"));
    if (!path.isEmpty())
...
{
    // 初始目录始终为当前 TIFF 文件所在目录
    const QString bmpDir = QFileInfo(m_tiffEdit->text()).absolutePath();
    const QString path = QFileDialog::getOpenFileName(
        this, "选择亮度图（可选）",
        bmpDir,
...
void TiffBmpPanel::onTiffPathChanged(const QString& path)
{
    ...
    // 仅当 TIFF 所在文件夹发生变化时，清空两个亮度路径 + 失效相关缓存
    // m_lastScannedFolder 在下方 scanFolder() 内更新，此处先比较旧值
    {
        ...
        {
            QSignalBlocker bb(m_bmpEdit);
            QSignalBlocker bf(m_bmpFolderEdit);
            m_bmpEdit->clear();
    ...
    const bool isGridFormat = applyGridFormatModeFromPath(path);

    scanFolder(path, true);
    if (!isGridFormat) {
        bool applied = false;
        if (m_autoTiffTagChk->isChecked())
            applied = tryApplyEmbeddedMetadata(path);
        if (m_autoResChk->isChecked() && !applied)
            tryParseResolutionFromFilename(path);
    }

    if (!isGridFormat && currentModeNeedsBmp()) {
...
void TiffBmpPanel::onDisplayModeChanged(int index)
{
    ...
    const QString t = m_tiffEdit->text().trimmed();
    if (currentModeNeedsBmp() && !t.isEmpty() && QFileInfo::exists(t)
        && m_bmpEdit->text().trimmed().isEmpty()
        && !isGridFormatPath(t))   // SRF/PLY/PCD：自带亮度或无亮度，不在外部目录里找
    {
...
void TiffBmpPanel::activateFileAtIndex(int index)
{
    ...
    // 目录内导航始终在同一文件夹，不清空亮度图路径

    const bool isGridFormat = applyGridFormatModeFromPath(tiff);

    if (!isGridFormat) {
        bool applied = false;
        if (m_autoTiffTagChk->isChecked())
            applied = tryApplyEmbeddedMetadata(tiff);
        if (m_autoResChk->isChecked() && !applied)
...
//   16-bit:  X<1, Y<1, Z<1
//   32-bit:  X<1, Y<1, Z<=1
// 返回 true 表示合法。is32Bit 来自 TIFF 元数据（is32Float / is96BitXYZ / is2Channel）。
static bool isResolutionValid(double rx, double ry, double rz, bool is32Bit)
{
    if (rx <= 0 || ry <= 0 || rz <= 0) return false;
    if (rx >= 1.0 || ry >= 1.0)        return false;
    return is32Bit ? (rz <= 1.0) : (rz < 1.0);
}

// 单字段合法性（用于 TIFF 标签的部分应用）
static bool isResAxisValid(double v, bool isZ, bool is32Bit)
{
    if (v <= 0) return false;
    if (!isZ)   return v < 1.0;
    return is32Bit ? (v <= 1.0) : (v < 1.0);
}
...
void TiffBmpPanel::tryParseResolutionFromFilename(const QString& path)
{
    ...
    bool is32Bit = false;
    {
        ...
        if (prepareTiffInfo(path, info, nullptr) && info.valid)
            is32Bit = info.is32Float || info.is96BitXYZ || info.is2Channel;
    }

    if (!isResolutionValid(rx, ry, rz, is32Bit)) {
        m_resWarningLabel->setVisible(false);
        const QString rule = is32Bit
            ? ls("32-bit: X<1 Y<1 Z≤1", "32-bit: X<1 Y<1 Z≤1")
            : ls("16-bit: X<1 Y<1 Z<1", "16-bit: X<1 Y<1 Z<1");
        m_statusLabel->setText(
            QString(ls("⚠ 文件名分辨率超限（X=%1 Y=%2 Z=%3 mm，要求 %4），已忽略",
                       "⚠ Filename res out of range (X=%1 Y=%2 Z=%3 mm, expect %4), ignored"))
                .arg(rx, 0, 'g', 4).arg(ry, 0, 'g', 4).arg(rz, 0, 'g', 4).arg(rule));
        m_statusLabel->setStyleSheet("color:#cc6600;");
        return;
    }
...
bool TiffBmpPanel::tryApplyEmbeddedMetadata(const QString& path)
{
    if (path.isEmpty() || !QFileInfo::exists(path))
        return false;
    ...
    const bool is32Bit = info.is32Float || info.is96BitXYZ || info.is2Channel;
    const bool xOk = info.hasEmbeddedResX && isResAxisValid(info.embeddedResX, false, is32Bit);
    const bool yOk = info.hasEmbeddedResY && isResAxisValid(info.embeddedResY, false, is32Bit);
    const bool zOk = info.hasEmbeddedResZ && isResAxisValid(info.embeddedResZ, true,  is32Bit);
    const bool anyRes = xOk || yOk || zOk;
    const bool anyOff = info.hasEmbeddedOffX || info.hasEmbeddedOffY || info.hasEmbeddedOffZ;
    if (!anyRes && !anyOff && !info.embeddedUInt16Plus32768)
        return false;

    // ── 分辨率（按位深规则过滤后逐字段应用）─────────────────────────────────
    if (xOk) { QSignalBlocker bx(m_resXSpin); m_resXSpin->setValue(info.embeddedResX); }
    if (yOk) { QSignalBlocker by(m_resYSpin); m_resYSpin->setValue(info.embeddedResY); }
    if (zOk) { QSignalBlocker bz(m_resZSpin); m_resZSpin->setValue(info.embeddedResZ); }

    // ── 偏移值仅填充 spinbox，不自动勾选"启用偏移"复选框（全程手动控制）──
    // 检测到 Storage=UInt16RawPlus32768 也不动复选框；reinterpretAsSignedZ 由用户决定
    if (info.hasEmbeddedOffX && m_offXSpin) {
        QSignalBlocker bo(m_offXSpin); m_offXSpin->setValue(info.embeddedOffX);
    }
    if (info.hasEmbeddedOffY && m_offYSpin) {
        QSignalBlocker bo(m_offYSpin); m_offYSpin->setValue(info.embeddedOffY);
    }
    if (info.hasEmbeddedOffZ && m_offZSpin) {
        QSignalBlocker bo(m_offZSpin); m_offZSpin->setValue(info.embeddedOffZ);
    }

    m_resWarningLabel->setVisible(false);

    // 状态栏提示来源（仅在分辨率或偏移有更新时显示）
    QStringList parts;
    if (xOk || yOk || zOk) {
        QStringList r;
        if (xOk) r << QString("X=%1").arg(info.embeddedResX, 0, 'g', 4);
        if (yOk) r << QString("Y=%1").arg(info.embeddedResY, 0, 'g', 4);
        if (zOk) r << QString("Z=%1").arg(info.embeddedResZ, 0, 'g', 4);
        parts << r.join(' ');
    }
    if (anyOff) {
        QStringList o;
        if (info.hasEmbeddedOffX) o << QString("OX=%1").arg(info.embeddedOffX, 0, 'g', 4);
        if (info.hasEmbeddedOffY) o << QString("OY=%1").arg(info.embeddedOffY, 0, 'g', 4);
        if (info.hasEmbeddedOffZ) o << QString("OZ=%1").arg(info.embeddedOffZ, 0, 'g', 4);
        parts << o.join(' ');
    }
    if (!parts.isEmpty()) {
        const QString src = info.embeddedMetaSource.isEmpty()
                                ? QStringLiteral("TIFF")
                                : info.embeddedMetaSource;
        m_statusLabel->setText(
            QString(ls("来自 TIFF[%1]: %2", "From TIFF[%1]: %2"))
                .arg(src, parts.join(QStringLiteral(" | "))));
        m_statusLabel->setStyleSheet("color:#0055cc;");
    }

    return anyRes || anyOff;
}
...
void TiffBmpPanel::scanFolder(const QString& tiffPath, bool forceRefresh)
{
    ...
    QStringList names = dir.entryList({"*.tif", "*.tiff", "*.srf", "*.sur", "*.ply", "*.pcd"}, QDir::Files, sf);
                                   ...
                                   int* outBitDepth)
{
    ...
    // SRF 文件：构造合成 TiffInfo（loader 内部仍走 SRF 分支，但 panel 需要 W/H/bitDepth）
    if (TiffBmpLoader::isSrfPath(tiffPath)) {
        TiffBmpLoader::SrfMeta meta;
        if (!TiffBmpLoader::parseSrfMeta(tiffPath, meta, nullptr) || !meta.valid)
            return false;
        TiffBmpLoader::TiffInfo info;
        info.valid     = true;
        info.width     = meta.width;
        info.height    = meta.length;
        info.bitDepth  = 32;        // 在 loader 内 baked 为 mm 浮点
        info.is32Float = true;
        constexpr int kMaxCacheEntries = 200;
        if (m_tiffInfoCache.size() >= kMaxCacheEntries)
            m_tiffInfoCache.clear();
        m_tiffInfoCache.insert(tiffPath, info);
        outInfo = info;
        if (outBitDepth)
            *outBitDepth = info.bitDepth;
        return true;
    }

    // SUR 文件：构造合成 TiffInfo（loader 内部仍走 SUR 分支，但 panel 需要 W/H/bitDepth）
    if (TiffBmpLoader::isSurPath(tiffPath)) {
        TiffBmpLoader::SurMeta meta;
        if (!TiffBmpLoader::parseSurMeta(tiffPath, meta, nullptr) || !meta.valid)
            return false;
        TiffBmpLoader::TiffInfo info;
        info.valid     = true;
        info.width     = meta.width;
    ...
    // PLY/PCD 文件：仅读头部拿点数（不解析顶点/不做网格重建），构造合成 TiffInfo
    // 真正的整档解析 + 网格重建延后到 load() 内部（后台线程），避免大点云文件卡 UI 线程
    if (TiffBmpLoader::isPlyPath(tiffPath) || TiffBmpLoader::isPcdPath(tiffPath)) {
        qint64 pointCount = 0;
        if (!TiffBmpLoader::peekPlyPcdPointCount(tiffPath, pointCount, nullptr))
            return false;
        TiffBmpLoader::TiffInfo info;
        info.valid     = true;
        info.width     = 0;
    ...
    if (info.valid) {
        // 上限保护：长时间浏览大量 TIFF 时清空缓存避免无限增长
        // 每条 TiffInfo 含若干 QVector<quint32>，单条几 KB 但累积会大
        constexpr int kMaxCacheEntries = 200;
        if (m_tiffInfoCache.size() >= kMaxCacheEntries)
            m_tiffInfoCache.clear();
        m_tiffInfoCache.insert(tiffPath, info);
...
bool TiffBmpPanel::applyGridFormatModeFromPath(const QString& path)
{
    const bool isSrf = TiffBmpLoader::isSrfPath(path);
    const bool isSur = TiffBmpLoader::isSurPath(path);
    const bool isGridFormat = isGridFormatPath(path);

    // 分辨率组：SRF/SUR/PLY/PCD 时禁用（仅显示；坐标与分辨率均由文件自带）
    if (m_resGrp) m_resGrp->setEnabled(!isGridFormat);
    if (m_resWarningLabel && isGridFormat)
        m_resWarningLabel->setVisible(false);

    // 亮度图相关控件 + 浏览按钮：SRF/SUR/PLY/PCD 时禁用
    if (m_bmpEdit)       m_bmpEdit->setEnabled(!isGridFormat);
    if (m_bmpFolderEdit) m_bmpFolderEdit->setEnabled(!isGridFormat);
    if (m_bmpBrowseBtn)  m_bmpBrowseBtn->setEnabled(!isGridFormat);
    if (m_bmpFolderBtn)  m_bmpFolderBtn->setEnabled(!isGridFormat);

    if (!isGridFormat)
        return false;

    // 清空亮度图路径（自带或无）
    if (m_bmpEdit && !m_bmpEdit->text().isEmpty()) {
        QSignalBlocker b(m_bmpEdit);
        m_bmpEdit->clear();
    }

    // SRF/SUR：把嵌入的 scale_x/y/z 写入分辨率框（仅显示，组已禁用）
    // PLY/PCD：是否为规则网格需要整档解析才能确定，此处（廉价预检阶段）无法预知，跳过
    if (isSrf && QFileInfo::exists(path)) {
        TiffBmpLoader::SrfMeta meta;
        if (TiffBmpLoader::parseSrfMeta(path, meta, nullptr) && meta.valid) {
            QSignalBlocker bx(m_resXSpin);
            QSignalBlocker by(m_resYSpin);
            QSignalBlocker bz(m_resZSpin);
            m_resXSpin->setValue(meta.scaleX);
            m_resYSpin->setValue(meta.scaleY);
            m_resZSpin->setValue(meta.scaleZ);
        }
    } else if (isSur && QFileInfo::exists(path)) {
        TiffBmpLoader::SurMeta meta;
        if (TiffBmpLoader::parseSurMeta(path, meta, nullptr) && meta.valid) {
            QSignalBlocker bx(m_resXSpin);
            QSignalBlocker by(m_resYSpin);
            QSignalBlocker bz(m_resZSpin);
            m_resXSpin->setValue(meta.scaleX);
...
void TiffBmpPanel::startAsyncLoad(const QString& tiffPath, quint64 generation)
{
    ...
    // 修复前生成的旧版16位无符号TIFF：zOffset 未按无符号换算，需 -32768 重新解释后才能配合
    // offZ 得到正确高度；修复后的新TIFF已直接可用 raw*resZ+offZ，默认不再重新解释。
    // 该兼容路径仅能通过隐藏快捷键 F1（焦点在本面板时）临时开启，见构造函数。
    const bool   reinterpretAsSignedZ = useOffset && m_legacyUint16CompatEnabled;
    ...
    m_loadWatcher->setFuture(QtConcurrent::run([=]() -> AsyncLoadResult {
        ...
        result.imageWidth  = preparedInfo.width;   // 像素尺寸，applyLoadResult 用于尺寸突变判断
        result.imageHeight = preparedInfo.height;

        TiffBmpLoader::TiffInfo infoCopy = preparedInfo;
        int bitDepth = preparedBitDepth;
        bool fallbackToUnorganized = false;
        result.object = TiffBmpLoader::load(
            ...
            offX, offY, offZ,
            reinterpretAsSignedZ,   // uint16 视为有符号（仅 F1 隐藏兼容开关 + 启用偏移时生效）
            infoCopy.valid ? &infoCopy : nullptr,
            &result.error,
            yStride,
            cloudUID,    // 预分配 UID，绕过后台线程中的 GetNextUniqueID() 调用
            meshUID,
            &fallbackToUnorganized);
        result.bitDepth = bitDepth;
        result.fallbackToUnorganizedCloud = fallbackToUnorganized;
...
void TiffBmpPanel::applyLoadResult(const AsyncLoadResult& result)
{
    ...
    // 图像像素尺寸相对上次变化超过 2× 或 0.5×：等同于点击复位视角
    if (!shouldReset
        && result.imageWidth > 0 && result.imageHeight > 0
        && m_prevImageW > 0 && m_prevImageH > 0)
    {
        const double rW = double(result.imageWidth)  / m_prevImageW;
        const double rH = double(result.imageHeight) / m_prevImageH;
        if (rW > 2.0 || rW < 0.5 || rH > 2.0 || rH < 0.5)
            shouldReset = true;
    }
    if (result.imageWidth > 0 && result.imageHeight > 0) {
        m_prevImageW = result.imageWidth;
        m_prevImageH = result.imageHeight;
    }
    ...
    // Z 显示放大：仅作用于本次新加载的对象（与"保留对象"逻辑一致，不追溯旧对象）
    applyZDisplayScale(object);
    ...
    // 用 redrawAll 而非 refreshAll：前者强制立刻进入 GL 上下文渲染，
    // 触发已删除对象的 QOpenGLBuffer 排队销毁立即执行，释放 GPU 显存。
    // refreshAll 只刷新当前可见数据，可能延迟回收旧 cloud 的 VBO。
    m_app->redrawAll();
    m_app->updateUI();

    // 旧点云已 delete + GL VBO 已通过 redrawAll 触发释放，
    // 这里强制堆压缩 + 工作集裁剪，把 Windows 任务管理器看到的内存归位。
    trimProcessMemory();
    ...
    const QString bitInfo   = (result.bitDepth == 128) ? ls(" [128bit双通道]", " [128-bit 2-ch]")
                            : (result.bitDepth == 96)  ? ls(" [96bit三通道XYZ]", " [96-bit 3-ch XYZ]")
    ...
    // PLY/PCD 非规则网格：已降级为普通无序点云，提示用户，并禁用"导出当前"
    // （TIFF 要求均匀间距，无序点云无法导出）
    if (result.fallbackToUnorganizedCloud) {
        m_statusLabel->setText(m_statusLabel->text() + " " + ls(
            "⚠ 非规则网格，已作为普通点云导入（网格/降噪/TIFF导出不可用）",
            "⚠ Non-grid point cloud — imported as unorganized cloud (mesh/denoise/TIFF export unavailable)"));
        m_statusLabel->setStyleSheet("color:#cc7a00;");
    }
    if (m_exportOneBtn)
        m_exportOneBtn->setEnabled(!result.fallbackToUnorganizedCloud);

    scheduleSettingsSave();
}

void TiffBmpPanel::applyZDisplayScale(ccHObject* obj)
{
    if (!obj || !m_zScaleSpin)
        return;

    const double z = m_zScaleSpin->value();

    // 网格光照开关：CC 渲染时法线和顶点共用同一个 GL 变换矩阵，而这个矩阵只对
    // 旋转+平移是正确的——非均匀缩放（仅放大 Z）下按同一矩阵变换法线在数学上
    // 是错的。曾尝试用逆转置公式手动预校正法线，但真实扫描数据几乎处处带有
    // 细微噪声级别的坡度差异，独立地对每个顶点的法线做"除以 z 再归一化"会把
    // 相邻顶点间原本极小的噪声差异非线性放大成方向相差悬殊的法线，渲染出大片
    // 裂纹状明暗噪点，比不修正更难看。真正稳妥的办法是放大时干脆不做光照：
    // 陡峭"侧面"发白、平坦顶部发黑都是光照按错误法线计算出来的伪影，关掉光照后
    // 网格改用纯顶点色（高度/亮度/融合色）平画显示，色彩本身不受此影响，
    // 只是失去光影层次感；倍数改回 1x 时恢复正常光照+法线着色。
    if (auto* mesh = dynamic_cast<ccMesh*>(obj))
        mesh->showNormals(qFuzzyCompare(z, 1.0));

    if (qFuzzyCompare(z, 1.0)) {
        obj->resetGLTransformation();
        return;
    }

    // 以对象自身（未叠加显示变换的）包围盒最低点为基准缩放 Z，而不是世界坐标原点：
    // 若直接绕 Z=0 缩放，物体自身的绝对 Z（如设置了 Z 偏移、或 SRF/SUR 自带非零基准）
    // 会让"低处"和"高处"产生不成比例的绝对位移错觉——低处离原点近、位移小，
    // 高处离原点远、位移大，整个物体看起来向一侧歪斜而不是从底部均匀拉伸。
    // 锚定在物体自身最低点后，最低点缩放后保持原位，起伏按倍数从该基准向上拉伸。
    const ccBBox bbox = obj->getOwnBB();
    const double zBase = bbox.isValid() ? static_cast<double>(bbox.minCorner().z) : 0.0;

    // 仅缩放 Z 轴对角线（矩阵第 10 个元素，列主序第 3 列第 3 行）+ Z 平移（第 14 个
    // 元素）实现"绕 zBase 缩放"：newZ = zBase + (oldZ - zBase) * z = oldZ*z + zBase*(1-z)
    // 纯渲染效果，不会被 CCCoreLib 的测距/体积等算法计入
    ccGLMatrix mat; // 默认构造即单位矩阵
    mat.data()[10] = static_cast<float>(z);
    mat.data()[14] = static_cast<float>(zBase * (1.0 - z));
    obj->setGLTransformation(mat);
}
...
void TiffBmpPanel::retranslateUi()
{
    ...
    // 导出 TIFF 区
    if (m_exportGrp)        m_exportGrp->setTitle(ls("TIFF 转换导出", "TIFF Export"));
    if (m_exportEnableBtn) {
        const bool on = m_exportEnableBtn->isChecked();
        const QString base = ls("TIFF 转换导出", "TIFF Export");
        m_exportEnableBtn->setText((on ? QStringLiteral("▲  ") : QStringLiteral("▼  ")) + base);
    }
    if (m_lblExportDirRow)  m_lblExportDirRow->setText(ls("输出目录:", "Output Dir:"));
    if (m_lblExportFmtRow)  m_lblExportFmtRow->setText(ls("格式:", "Format:"));
    if (m_exportInvalidLbl) m_exportInvalidLbl->setText(ls("无效填充:", "Invalid Fill:"));
    if (m_exportOneBtn)     m_exportOneBtn->setText(ls("导出当前", "Export Current"));
    if (m_exportAllBtn)     m_exportAllBtn->setText(ls("自动遍历导出", "Auto Batch Export"));
    if (m_exportFmtCb) {
        const int idx = m_exportFmtCb->currentIndex();
        QSignalBlocker b(m_exportFmtCb);
        m_exportFmtCb->clear();
        m_exportFmtCb->addItem(ls("16-bit 无符号", "16-bit Unsigned"), 0);
        m_exportFmtCb->addItem(ls("16-bit 有符号", "16-bit Signed"),   1);
        m_exportFmtCb->addItem(ls("32-bit 浮点",   "32-bit Float"),    2);
        m_exportFmtCb->setCurrentIndex(qBound(0, idx, 2));
    }
    if (m_exportDirEdit)
        m_exportDirEdit->setPlaceholderText(ls("默认 = 当前文件目录 / 转换TIFF输出目录",
                                               "Default = current file dir / 转换TIFF输出目录"));
    ...
    m_tiffEdit->setPlaceholderText(ls("选择或拖拽 TIFF / SRF / SUR / PLY / PCD 文件...", "Select or drag TIFF / SRF / SUR / PLY / PCD file..."));
    ...
    m_autoResChk->setText(ls("自动从文件名", "From filename"));
    if (m_autoTiffTagChk)
        m_autoTiffTagChk->setText(ls("自动从TIFF标签", "From TIFF tags"));
    ...
    if (m_lblZScaleText) m_lblZScaleText->setText(ls("Z显示放大:", "Z Display Scale:"));
    if (m_zScaleResetBtn) {
        m_zScaleResetBtn->setText(ls("恢复1x", "Reset 1x"));
        m_zScaleResetBtn->setToolTip(ls("一键恢复 Z 显示放大为 1×（不放大）",
                                        "Reset Z display scale to 1x (no exaggeration)"));
    }
    {
    ...
    const QString readyZh = QString::fromUtf8("就绪 — 支持拖拽 TIFF / SRF / SUR / PLY / PCD 文件");
    const QString readyEn = QString::fromUtf8("Ready — drag & drop TIFF / SRF / SUR / PLY / PCD files");
    const QString readyZhOld1 = QString::fromUtf8("就绪 — 支持拖拽 TIFF / SRF / PLY / PCD 文件");
    const QString readyEnOld1 = QString::fromUtf8("Ready — drag & drop TIFF / SRF / PLY / PCD files");
    const QString readyZhOld = QString::fromUtf8("就绪 — 支持拖拽 TIFF / SRF 文件");
    const QString readyEnOld = QString::fromUtf8("Ready — drag & drop TIFF / SRF files");
    const QString readyZhOld2 = QString::fromUtf8("就绪 — 支持拖拽 TIFF 文件");
    const QString readyEnOld2 = QString::fromUtf8("Ready — drag & drop TIFF files");
    if (m_statusLabel->text() == readyZh     || m_statusLabel->text() == readyEn
     || m_statusLabel->text() == readyZhOld1 || m_statusLabel->text() == readyEnOld1
     || m_statusLabel->text() == readyZhOld  || m_statusLabel->text() == readyEnOld
     || m_statusLabel->text() == readyZhOld2 || m_statusLabel->text() == readyEnOld2)
        m_statusLabel->setText(ls("就绪 — 支持拖拽 TIFF / SRF / SUR / PLY / PCD 文件",
                                  "Ready — drag & drop TIFF / SRF / SUR / PLY / PCD files"));
...
void TiffBmpPanel::loadSettings()
{
    ...
    // 旧版只有 autoRes（合一开关）；按它初始化新分裂的两个开关，
    // 之后单独保存 autoResFilename / autoResTiffTag 时各自覆盖
    // 两个开关互斥：最多只能勾一个。若旧配置同时为 true，优先保留 TIFF 标签
    {
        const bool legacy = s.value("autoRes", false).toBool();
        bool fromName = s.value("autoResFilename", legacy).toBool();
        bool fromTag  = s.value("autoResTiffTag",  legacy).toBool();
        if (fromName && fromTag) fromName = false;
        m_autoResChk->setChecked(fromName);
        m_autoTiffTagChk->setChecked(fromTag);
    }
    ...
    m_zScaleSpin->setValue(s.value("zDisplayScale", 1.0).toDouble());
    ...
    // 导出 TIFF
    if (m_exportEnableBtn) {
        const bool en = s.value("Export/enabled", false).toBool();
        m_exportEnableBtn->setChecked(en);   // toggled 信号会同步 m_exportGrp 与按钮文字
        if (m_exportGrp) m_exportGrp->setVisible(en);
    }
    if (m_exportDirEdit)
        m_exportDirEdit->setText(s.value("Export/dir", "").toString());
    if (m_exportFmtCb) {
        const int fmt = qBound(0, s.value("Export/format", 0).toInt(), 2);
        m_exportFmtCb->setCurrentIndex(fmt);
        if (m_exportInvalidLbl)  m_exportInvalidLbl->setVisible(fmt == 2);
        if (m_exportInvalidSpin) m_exportInvalidSpin->setVisible(fmt == 2);
    }
    if (m_exportInvalidSpin)
        m_exportInvalidSpin->setValue(s.value("Export/invalid32", -100.0).toDouble());
    ...
    if (!tiffPath.isEmpty() && QFileInfo::exists(tiffPath))
    {
        // 启动时仅恢复路径文本：屏蔽 textChanged 信号，避免 onTiffPathChanged
        // 触发 tryApplyEmbeddedMetadata（同步 IFD 解析）和 triggerAutoReload
        // （400ms 后自动 onLoad）。某些畸形/不支持格式的 TIFF 在解析或解码
        // 阶段可能触发 SIGSEGV，导致 TiffViewer 启动即崩溃、界面无法打开。
        // 启动后用户需点击"加载"或导航按钮主动加载文件。
        {
            QSignalBlocker blocker(m_tiffEdit);
            m_tiffEdit->setText(tiffPath);
        }
        // 仍可安全地扫描所在目录以填充导航条
        scanFolder(tiffPath, true);
        m_statusLabel->setText(ls(
            "已恢复上次路径，点击\"加  载\"或导航按钮以加载",
            "Last path restored — click \"Load\" or use navigation"));
        m_statusLabel->setStyleSheet("color:#0055cc;");

        // 启动时如果恢复的是 SRF/PLY/PCD 路径，统一切换到对应 UI 模式
        applyGridFormatModeFromPath(tiffPath);
    }
...
void TiffBmpPanel::saveSettings()
{
    ...
    s.setValue("autoResFilename", m_autoResChk->isChecked());
    s.setValue("autoResTiffTag",  m_autoTiffTagChk->isChecked());
    ...
    s.setValue("zDisplayScale",   m_zScaleSpin->value());
    ...
    if (m_exportEnableBtn)   s.setValue("Export/enabled",  m_exportEnableBtn->isChecked());
    if (m_exportDirEdit)     s.setValue("Export/dir",      m_exportDirEdit->text());
    if (m_exportFmtCb)       s.setValue("Export/format",   m_exportFmtCb->currentIndex());
    if (m_exportInvalidSpin) s.setValue("Export/invalid32",m_exportInvalidSpin->value());
    s.setValue("panelDefaultsVersion", kPanelDefaultsVersion);
    s.remove("computeNormals");
    s.sync();
}
...
QString TiffBmpPanel::extractExportStem(const QString& srcPath) const
{
    const QString full = QFileInfo(srcPath).completeBaseName(); // 不含扩展名
    // 匹配 "xxx_(16位无符号|16位有符号|32位)TIFF_X_Y_Z" → 取 xxx
    static const QRegularExpression re(
        QStringLiteral("^(.+?)_(?:16位无符号|16位有符号|32位)TIFF_[0-9.eE+\\-]+_[0-9.eE+\\-]+_[0-9.eE+\\-]+$"));
    const QRegularExpressionMatch m = re.match(full);
    if (m.hasMatch())
        return m.captured(1);
    return full;
}

QString TiffBmpPanel::ensureExportDir() const
{
    QString base = m_exportDirEdit ? m_exportDirEdit->text().trimmed() : QString();
    if (base.isEmpty()) {
        // 默认：当前 TIFF/SRF 所在目录
        const QString tiff = m_tiffEdit->text().trimmed();
        if (!tiff.isEmpty() && QFileInfo::exists(tiff))
            base = QFileInfo(tiff).absolutePath();
    }
    if (base.isEmpty())
        return QString();

    // 在 base 下追加一级 "转换TIFF输出目录"
    QDir d(base);
    const QString sub = QStringLiteral("转换TIFF输出目录");
    if (!d.exists(sub) && !d.mkpath(sub))
        return QString();
    return d.absoluteFilePath(sub);
}

TiffBmpLoader::ExportTiffMeta TiffBmpPanel::currentPanelMeta() const
{
    TiffBmpLoader::ExportTiffMeta m;

    // SRF：直接读 SRF 头部（面板对 SRF 禁用了偏移控件，spin 值不一定反映真实偏移）
    const QString src = m_tiffEdit ? m_tiffEdit->text().trimmed() : QString();
    if (!src.isEmpty() && TiffBmpLoader::isSrfPath(src) && QFileInfo::exists(src)) {
        TiffBmpLoader::SrfMeta s;
        if (TiffBmpLoader::parseSrfMeta(src, s, nullptr) && s.valid) {
            m.scaleX = s.scaleX;   m.scaleY = s.scaleY;   m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
        }
    }

    // SUR：同理，直接读 SUR 头部
    if (!src.isEmpty() && TiffBmpLoader::isSurPath(src) && QFileInfo::exists(src)) {
        TiffBmpLoader::SurMeta s;
        if (TiffBmpLoader::parseSurMeta(src, s, nullptr) && s.valid) {
            m.scaleX = s.scaleX;   m.scaleY = s.scaleY;   m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
        }
    ...
    // PLY/PCD（规则网格）：面板分辨率/偏移控件已禁用，需重新解析网格得到真实 scale/offset
    if (!src.isEmpty() && (TiffBmpLoader::isPlyPath(src) || TiffBmpLoader::isPcdPath(src))
        && QFileInfo::exists(src))
    {
        TiffBmpLoader::ExportSourceData s;
        if (TiffBmpLoader::readSourceForExport(src, s, nullptr) && s.valid) {
            m.scaleX = s.scaleX;   m.scaleY = s.scaleY;   m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
        }
    ...
    // TIFF：用面板当前 spin 值（反映用户输入时的 mm 数值）
    m.scaleX = m_resXSpin ? m_resXSpin->value() : 1.0;
    m.scaleY = m_resYSpin ? m_resYSpin->value() : 1.0;
    m.scaleZ = m_resZSpin ? m_resZSpin->value() : 1.0;
    if (m_useOffsetChk && m_useOffsetChk->isChecked()
        && m_offXSpin && m_offYSpin && m_offZSpin)
    {
        m.offsetX = m_offXSpin->value();
        m.offsetY = m_offYSpin->value();
        m.offsetZ = m_offZSpin->value();
    }
    return m;
}

TiffBmpLoader::ExportTiffMeta TiffBmpPanel::resolveExportMetaForFile(const QString& srcPath) const
{
    // SRF：始终从头部读取（最可靠）
    if (TiffBmpLoader::isSrfPath(srcPath)) {
        TiffBmpLoader::SrfMeta s;
        if (TiffBmpLoader::parseSrfMeta(srcPath, s, nullptr) && s.valid) {
            TiffBmpLoader::ExportTiffMeta m;
            m.scaleX = s.scaleX; m.scaleY = s.scaleY; m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
        }
        return currentPanelMeta();
    }

    // SUR：与 SRF 同理，始终从头部读取
    if (TiffBmpLoader::isSurPath(srcPath)) {
        TiffBmpLoader::SurMeta s;
        if (TiffBmpLoader::parseSurMeta(srcPath, s, nullptr) && s.valid) {
            TiffBmpLoader::ExportTiffMeta m;
            m.scaleX = s.scaleX; m.scaleY = s.scaleY; m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
    ...
    // PLY/PCD（规则网格）：与 SRF 同理，始终从重新解析的网格结果读取
    if (TiffBmpLoader::isPlyPath(srcPath) || TiffBmpLoader::isPcdPath(srcPath)) {
        TiffBmpLoader::ExportSourceData s;
        if (TiffBmpLoader::readSourceForExport(srcPath, s, nullptr) && s.valid) {
            TiffBmpLoader::ExportTiffMeta m;
            m.scaleX = s.scaleX;   m.scaleY = s.scaleY;   m.scaleZ = s.scaleZ;
            m.offsetX = s.offsetX; m.offsetY = s.offsetY; m.offsetZ = s.offsetZ;
            return m;
    ...
    // TIFF：嵌入元数据 + 文件名 _X_Y_Z 后缀；逐字段独立来源，避免"只有 X 嵌入"
    // 导致 Z 兜底为默认 1.0 的 bug
    TiffBmpLoader::TiffInfo info;
    const bool hasInfo = TiffBmpLoader::detectBitDepth(srcPath, &info) > 0 && info.valid;
    const bool is32Bit = hasInfo && (info.is32Float || info.is96BitXYZ || info.is2Channel);

    bool   embX = false, embY = false, embZ = false;
    double eX = 1.0, eY = 1.0, eZ = 1.0;
    bool   embOffX = false, embOffY = false, embOffZ = false;
    double oX = 0.0, oY = 0.0, oZ = 0.0;
    if (hasInfo) {
        if (info.hasEmbeddedResX) { eX = info.embeddedResX; embX = true; }
        if (info.hasEmbeddedResY) { eY = info.embeddedResY; embY = true; }
        if (info.hasEmbeddedResZ) { eZ = info.embeddedResZ; embZ = true; }
        if (info.hasEmbeddedOffX) { oX = info.embeddedOffX; embOffX = true; }
        if (info.hasEmbeddedOffY) { oY = info.embeddedOffY; embOffY = true; }
        if (info.hasEmbeddedOffZ) { oZ = info.embeddedOffZ; embOffZ = true; }
    }

    // 文件名末尾 3 段下划线分隔数值（与 panel 的 tryParseResolutionFromFilename 相同规则）
    bool   fnOk = false;
    double fnX = 1.0, fnY = 1.0, fnZ = 1.0;
    {
        const QString base = QFileInfo(srcPath).completeBaseName();
        const QStringList parts = base.split('_');
        if (parts.size() >= 3) {
            bool ok1, ok2, ok3;
            const double rx = parts[parts.size()-3].toDouble(&ok1);
            const double ry = parts[parts.size()-2].toDouble(&ok2);
            const double rz = parts[parts.size()-1].toDouble(&ok3);
            if (ok1 && ok2 && ok3 && isResolutionValid(rx, ry, rz, is32Bit)) {
                fnOk = true; fnX = rx; fnY = ry; fnZ = rz;
            }
        }
    }

    // 至少一个来源有效：按字段独立组合（嵌入优先；缺字段用文件名）
    if (embX || embY || embZ || embOffX || embOffY || embOffZ || fnOk) {
        TiffBmpLoader::ExportTiffMeta m;
        m.scaleX  = embX    ? eX : (fnOk ? fnX : m.scaleX);
        m.scaleY  = embY    ? eY : (fnOk ? fnY : m.scaleY);
        m.scaleZ  = embZ    ? eZ : (fnOk ? fnZ : m.scaleZ);
        if (embOffX) m.offsetX = oX;
        if (embOffY) m.offsetY = oY;
        if (embOffZ) m.offsetZ = oZ;
        return m;
    }
...
bool TiffBmpPanel::exportSourceTo(const QString& srcPath, const QString& outDir,
                                   int format, float invalidFill32,
                                   const TiffBmpLoader::ExportTiffMeta& meta,
                                   QString* outError)
{
    if (srcPath.isEmpty() || !QFileInfo::exists(srcPath)) {
        if (outError) *outError = "源文件不存在";
        return false;
    }
    if (outDir.isEmpty()) {
        if (outError) *outError = "输出目录无效";
        return false;
    }

    TiffBmpLoader::ExportSourceData src;
    QString err;
    if (!TiffBmpLoader::readSourceForExport(srcPath, src, &err)) {
        if (outError) *outError = err;
        return false;
    }

    // 文件名：原始 stem（去掉旧的 _XXTIFF_X_Y_Z 后缀）+ 格式标签 + 调用方提供的 mm 分辨率
    const QString stem = extractExportStem(srcPath);
    const QString fmtTag = (format == 0) ? "16位无符号TIFF"
                         : (format == 1) ? "16位有符号TIFF"
                                         : "32位TIFF";
    auto fmtRes = [](double v) -> QString {
        return QString::number(v, 'g', 6);
    };
    const QString tiffName = QString("%1_%2_%3_%4_%5.tif")
        .arg(stem, fmtTag, fmtRes(meta.scaleX), fmtRes(meta.scaleY), fmtRes(meta.scaleZ));
    const QString tiffPath = QDir(outDir).absoluteFilePath(tiffName);

    // 若是 SRF 源，meta 来源应为 SRF 自身（包含 offsetZ）；
    // currentPanelMeta() 对 SRF 已自动读取 SRF 头部，外部传入的 meta 同样可信
    bool ok = false;
    if (src.sourceIsFloat) {
        // 源是 32-bit float TIFF：无法无损转 int16，直接走 float 路径
        if (format == 2) {
            ok = TiffBmpLoader::writeTiffFloat32FromFloat(tiffPath, src.W, src.H,
                                                          src.zMm.data(), meta,
                                                          invalidFill32, &err);
        } else {
            // 16-bit 目标：用 meta 反算 raw_int16
            const qint64 N = static_cast<qint64>(src.W) * src.H;
            std::vector<qint16> tmp(static_cast<size_t>(N));
            const double sZ = (meta.scaleZ != 0.0) ? meta.scaleZ : 1.0;
            const double oZ = meta.offsetZ;
            for (qint64 i = 0; i < N; ++i) {
                const float v = src.zMm[i];
                if (std::isnan(v)) { tmp[i] = std::numeric_limits<qint16>::min(); continue; }
                const double r = (v - oZ) / sZ;
                tmp[i] = static_cast<qint16>(std::lround(
                    std::max(-32767.0, std::min(32767.0, r))));
            }
            ok = (format == 0)
                ? TiffBmpLoader::writeTiffUInt16(tiffPath, src.W, src.H, tmp.data(), meta, &err)
                : TiffBmpLoader::writeTiffInt16 (tiffPath, src.W, src.H, tmp.data(), meta, &err);
        }
    } else {
    ...
    // 内嵌亮度（SRF v2 / 2-channel TIFF）→ 写 "stem_.bmp"，带与 TIFF 一致的 DPI
    if (!src.intensity.empty()
        && static_cast<int>(src.intensity.size()) == src.W * src.H)
    {
        const QString bmpPath = QDir(outDir).absoluteFilePath(stem + "_.bmp");
        TiffBmpLoader::writeBmpGray(bmpPath, src.W, src.H,
                                     src.intensity.data(),
                                     meta.scaleX, meta.scaleY, nullptr);
    }
    return true;
}

void TiffBmpPanel::onExportCurrent()
{
    const QString src = m_tiffEdit->text().trimmed();
    if (src.isEmpty() || !QFileInfo::exists(src)) {
        m_statusLabel->setText(ls("❌ 当前未加载有效文件，无法导出",
                                  "❌ No valid file loaded — cannot export"));
        m_statusLabel->setStyleSheet("color:red;");
        return;
    }
    const QString dir = ensureExportDir();
    if (dir.isEmpty()) {
        m_statusLabel->setText(ls("❌ 输出目录创建失败", "❌ Failed to create output directory"));
        m_statusLabel->setStyleSheet("color:red;");
        return;
    }
    const int format = m_exportFmtCb ? m_exportFmtCb->currentIndex() : 0;
    const float inv32 = m_exportInvalidSpin
        ? static_cast<float>(m_exportInvalidSpin->value()) : -100.f;
    // 单次导出：使用面板当前 mm 值（反映用户输入时的分辨率）
    const TiffBmpLoader::ExportTiffMeta meta = currentPanelMeta();

    QString err;
    if (!exportSourceTo(src, dir, format, inv32, meta, &err)) {
        m_statusLabel->setText(QString(ls("❌ 导出失败: %1", "❌ Export failed: %1")).arg(err));
        m_statusLabel->setStyleSheet("color:red;");
        return;
    }

    // 同时复制外部亮度图（如有）—— 用 stem 命名，写入与 TIFF 一致的 DPI
    const QString bmpSrc = m_bmpEdit ? m_bmpEdit->text().trimmed() : QString();
    if (!bmpSrc.isEmpty() && QFileInfo::exists(bmpSrc)) {
        const QString stem = extractExportStem(src);
        const QString bmpOut = QDir(dir).absoluteFilePath(stem + "_.bmp");
        QImage img(bmpSrc);
        if (!img.isNull()) {
            img = img.convertToFormat(QImage::Format_Grayscale8);
            img.save(bmpOut, "BMP");
            // 写 DPI（基于面板当前 mm 值，保证和导出的 TIFF 缩略图比例一致）
            TiffBmpLoader::writeBmpResolutionInPlace(bmpOut, meta.scaleX, meta.scaleY);
        }
    }

    m_statusLabel->setText(QString(ls("✓ 已导出: %1 → %2",
                                      "✓ Exported: %1 → %2"))
        .arg(QFileInfo(src).fileName(), dir));
    m_statusLabel->setStyleSheet("color:#0a7a0a;");
}

void TiffBmpPanel::onExportAll()
{
    if (m_folderFiles.isEmpty()) {
        m_statusLabel->setText(ls("❌ 当前文件夹无 TIFF / SRF / SUR / PLY / PCD 文件",
                                  "❌ Current folder has no TIFF / SRF / SUR / PLY / PCD files"));
        m_statusLabel->setStyleSheet("color:red;");
        return;
    }
    const QString dir = ensureExportDir();
    ...
    int ok = 0, failed = 0;
    QString lastErr;
    const int total = m_folderFiles.size();
    for (int i = 0; i < total; ++i) {
        const QString src = m_folderFiles[i];
        m_statusLabel->setText(
            QString(ls("批量导出 %1/%2: %3", "Batch export %1/%2: %3"))
                .arg(i + 1).arg(total).arg(QFileInfo(src).fileName()));
        m_statusLabel->setStyleSheet("color:#0055cc;");
        qApp->processEvents();
        // 批量：逐文件解析 mm 元数据
        const TiffBmpLoader::ExportTiffMeta meta = resolveExportMetaForFile(src);
        QString err;
        if (exportSourceTo(src, dir, format, inv32, meta, &err)) ++ok;
        else { ++failed; lastErr = err; }
    }
    m_statusLabel->setText(QString(ls("✓ 批量完成: 成功 %1，失败 %2 → %3%4",
                                       "✓ Done: %1 ok, %2 failed → %3%4"))
        .arg(ok).arg(failed).arg(dir)
        .arg(failed ? QString(ls("（最后错误: %1）", " (last error: %1)")).arg(lastErr) : QString()));
    m_statusLabel->setStyleSheet(failed ? "color:#cc6600;" : "color:#0a7a0a;");
}
